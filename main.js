...
function search(e) {
  ...
  pasuje = all_tasks.filter(
    (element) => !element.textContent.split(" ")[0].includes(value)
  );
...
function checkToDoLength() {
  const tasksToDo = [...document.querySelector("#field").children];
  const tasksDone = [...document.querySelector("#field_done").children];
  ...
  if (tasksDone.length === 0) {
    document.querySelector("#field_done").style.opacity = "0";
    console.log(tasksDone.length);
  } else {
    document.querySelector("#field_done").style.opacity = "1";
    console.log(tasksDone.length);
  }
}
function addNewTask() {
